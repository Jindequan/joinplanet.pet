...
GET    /api/v1/pets/{id}/export                                                             → {export_version, exported_at, pet, profile, medications, tasks, task_logs, timeline}（成员可导出；不含 token/附件）
PATCH  /api/v1/pets/{id}           {..., version}                                           → {pet}（乐观锁；weight_g 拒绝——走 weight 事件）
DELETE /api/v1/pets/{id}           {confirm: <pet_id>}                                      → 204 硬删（owner）
PATCH  /api/v1/pets/{id}/profile   {allergies, conditions, emergency_contacts, med_decision_maker, notes} → {profile}（省略字段保留；空数组/空字符串显式清除）
...
类型注册表（v1）：`symptom`/`weight`（须 weight_g，同步宠物体重）/`medication`（仅自动）/`vaccine`/`vet_visit`/`note`。V1 不接收图片、附件或文档；B10 的 `transfer` 仅由系统自动写入。
