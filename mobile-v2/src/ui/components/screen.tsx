...
export function Screen({ children, scroll = false, contentContainerStyle, ...props }: ViewProps & { scroll?: boolean; contentContainerStyle?: ScrollViewProps['contentContainerStyle'] }) {
  ...
  const content = scroll ? <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[screenStyles.scrollContent, { paddingTop: theme.spacing.page, paddingHorizontal: theme.spacing.page, paddingBottom: theme.spacing.bottomClearance }, contentContainerStyle]}>{children}</ScrollView> : <View style={[screenStyles.content, { paddingTop: theme.spacing.page, paddingHorizontal: theme.spacing.page, paddingBottom: theme.spacing.page, gap: theme.spacing.md }, contentContainerStyle]}>{children}</View>;
