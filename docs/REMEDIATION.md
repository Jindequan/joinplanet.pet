...
- 2026-09-15 摘要内容选择补齐：健康摘要创建表单现在逐项控制宠物档案、当前用药和近期记录，并把真实 `sections` 写入分享选项；取消全部内容会被拦截，避免生成没有可读内容的摘要。创建前新增摘要预览，可返回修改主诉、范围和隐私选项。桌面/移动摘要分享回归 2/2、TypeScript 与 Lint 通过。
