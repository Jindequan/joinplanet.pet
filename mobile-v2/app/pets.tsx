...
import { useAccessiblePets, useFamilies, useInvalidateApi, useTodayForFamilies } from '../src/core/query/hooks';
...
function PetsRoute() {
  ...
  const directPetIds = accessiblePets.pets
    .filter((pet) => !(pet.family_ids ?? []).some((familyId) => familyIds.includes(familyId)))
    .map((pet) => pet.id);
  const today = useTodayForFamilies(familyIds, '', directPetIds);
  const [activeFamilyId, setActiveFamilyId] = useState<string>();
  useEffect(() => {
    if (typeof params.familyId === 'string' && familyIds.includes(params.familyId)) setActiveFamilyId(params.familyId);
  }, [familyIds, params.familyId]);
  const createIntent = useIdempotencyKey();
  const family = families.data?.families.find((item) => item.id === activeFamilyId) ?? families.data?.families[0] ?? { id: '', name: 'your care space', timezone: '', created_at: '', role: 'viewer' as const };
  const [adding, setAdding] = useState(params.add === '1');
  const canCreatePet = Boolean(family && family.role !== 'viewer' && family.role !== 'read_only');
  ...
  const create = useMutation({
    mutationFn: () => planetApi.pets.create(family!.id, petPayload({ name, species, breed, birth_date: dateKey(birthDate), sex, neutered, weight_g: weightG }), createIntent.current()),
    onSuccess: (result) => { createIntent.reset(); setName(''); setBreed(''); setBirthDate(null); setSex(''); setNeutered(false); setWeightG(''); setSpecies('dog'); setAdding(false); invalidate.pets(family!.id); invalidate.families(); router.push({ pathname: '/(tabs)/pet', params: { petId: result.pet.id, intent: 'care' } }); },
  ...
  const retryPets = () => { void families.refetch(); void accessiblePets.refetch(); void today.refetch(); };
  const blockingError = (families.isError && !families.data) || (accessiblePets.isError && !accessiblePets.hasData);
  const hasStaleData = Boolean((families.isError && families.data) || (accessiblePets.isError && accessiblePets.hasData) || (today.isError && today.hasData));
  ...
  const list = useMemo(() => {
    const visibleFamilyId = activeFamilyId ?? (typeof params.familyId === 'string' ? params.familyId : undefined);
    if (!visibleFamilyId) return accessiblePets.pets;
    return accessiblePets.pets.filter((pet) => (pet.family_ids ?? []).includes(visibleFamilyId));
  }, [accessiblePets.pets, activeFamilyId, params.familyId]);
  if (families.isLoading || accessiblePets.isLoading) return <Screen><LoadingState label="Loading your Pets" /></Screen>;
