...
import { AppText, Button, Card, Screen, SegmentedControl, TextField } from '../src/ui/components';
...
export default function PetsRoute() {
  ...
  const create = useMutation({
    mutationFn: () => planetApi.pets.create(circle!.id, petPayload({ name, species, breed, birth_date: '', sex: '', neutered: false, weight_g: '' })),
    onSuccess: (result) => { setName(''); setBreed(''); setSpecies('dog'); setAdding(false); invalidate.pets(circle!.id); invalidate.circles(); router.push({ pathname: '/(tabs)/pet', params: { petId: result.pet.id } }); },
    onError: (err) => setError(err instanceof ApiError ? err.message : 'Unable to add this Pet.'),
  });
  function submitPet() {
    const parsed = petSchema.safeParse({ name, species, breed, birth_date: '', sex: '', neutered: false, weight_g: '' });
