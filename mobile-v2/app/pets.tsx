...
function PetCard({ pet, intent }: { pet: Pet; intent?: string }) {
  const { theme } = useTheme();
  return <Pressable accessibilityRole="button" accessibilityLabel={intent === 'care' ? `Add care for ${pet.name}` : intent === 'export' ? `Export ${pet.name}` : `Open ${pet.name}`} onPress={() => router.push({ pathname: '/(tabs)/pet', params: { petId: pet.id, intent: intent === 'care' || intent === 'export' ? intent : undefined } })} style={({ pressed }) => [styles.petCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }, pressed && { opacity: theme.motion.pressOpacity }]}>
...
export default function PetsRoute() {
  ...
  const retryPets = () => { void circles.refetch(); void accessiblePets.refetch(); };
  const blockingError = (circles.isError && !circles.data) || (accessiblePets.isError && !accessiblePets.hasData);
  const hasStaleData = Boolean((circles.isError && circles.data) || (accessiblePets.isError && accessiblePets.hasData));
