...
六个系统的主链（状态机、终态、幂等、同事务守卫）全部具备明确设计且能在代码逐行指认——「有完整业务设计」成立。离「零漏洞」的距离集中在**系统边界的收束**（注销对他人的连带、终局迁移对 pending 请求的处置、调度触达的装配）与少量**生命周期终态语义缺失**（宠物离世）。全部 P0/P1/P2 已在本批修复；产品语义级缺口已于 2026-09-22 由 founder 裁决处置（结果见第四节；DEFECT-LEDGER §八 L36–L45）。
...
founder 2026-09-22 逐条裁决（详见 [DEFECT-LEDGER §八](../../DEFECT-LEDGER.md) L36–L45；代码批 commit 由本体回填）：

1. **宠物离世语义**（L36）：裁决为新增 deceased 特殊状态，本批落地（占配额、数据保留只读、仅可删除、无恢复）；纪念功能延后。
2. 宠物 30 天恢复窗（L37）：裁决为真窗口+purge，本批落地。
3. once 提醒语义（L38）：裁决为 once 必须带时间，本批落地，不再存在「无时段且已指派」形态。
4. viewer 可被移交所有权 / 邀请码独立撤销（L39）：裁决为不改动（人的行为），销号。
5. digest 通道 / 角标 / 送达窗口（L40）：摘要邮件默认停发（DIGEST_EMAIL_ENABLED 开关）；推送为必须（已由本批调度推送修复达成）；角标与时区窗口维持现状登记。
6. 「照片断网入队」承诺 vs 实现（L41）：裁决为照片离线队列不做（登记之后版本）；文档口径已按实现修正（PRODUCT.md §6.4：文字类记录断网入离线队列，照片在线直传 R2）。
7. 导出口径落字（L42）：导出 MVP 搁置（之后版本项）；access-grants（L43）=预留能力（仅 family，API 保留，无产品入口）；纪念宠转移 UI（L44）维持登记，随 deceased 落地复核休眠归档转移口径。
