...
export function Screen({ children, scroll = false, contentContainerStyle, ...props }: ViewProps & { scroll?: boolean; contentContainerStyle?: ScrollViewProps['contentContainerStyle'] }) {
  ...
  return <SafeAreaView edges={['top', 'left', 'right']} style={[screenStyles.safe, { backgroundColor: theme.colors.background }]}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : Platform.OS === 'android' ? 'height' : undefined} style={screenStyles.flex} {...props}>{content}</KeyboardAvoidingView></SafeAreaView>;
}
