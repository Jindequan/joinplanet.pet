...
- 2026-09-15 邀请回跳体验补强：公开链接和登录后带邀请码回跳均不再自动弹出键盘，先展示邀请码预览与主 CTA；只有没有入口邀请码时才自动聚焦输入框。桌面/移动邀请 E2E 6/6 通过。
- 2026-09-15 批量责任链恢复补强：批量拒绝后的继续转交在详情读取失败、后台刷新失败或权限依赖失败时都不再静默吞错，批量页面保留可读错误和“重试打开继续安排”；桌面/移动 E2E 2/2、前端契约新增检查均通过。
- 2026-09-15 宠物转移状态保护：已有转移请求查询失败时不再把空结果当成“没有请求”，页面阻止新转移并提供重试，避免网络异常造成重复转移；桌面/移动 E2E 4/4 和前端契约检查通过。
- 2026-09-15 家庭转移请求恢复补强：家庭详情页的收到的宠物转移查询失败时不再静默显示空列表，管理员会看到“转移请求暂时无法加载”和“重试加载转移请求”；连续 503 + 重试入口桌面/移动 E2E 2/2、TypeScript、Lint、前端契约检查均通过。
- 2026-09-15 摘要打印路径修复：Web 端“打印 / 保存 PDF”现在把生成的 Vet-ready A4 摘要写入隐藏打印文档并调用该文档的打印上下文，不再把当前 App 页面误打印；重复点击不会累积隐藏 iframe。摘要打印 E2E、全量 E2E 94/94、TypeScript、Lint、原生 Simulator Debug 构建与 CI `34908561802` 均通过。
- 2026-09-15 转移对象识别补强：转移 API 同时返回来源/目标家庭名称，家庭转移页按方向显示“来自/转往”家庭，避免用户只看到宠物名和不可读 ID；真实 API 集成断言与桌面/移动 E2E 覆盖双方名称。
...
- 2026-09-15 准备就诊直达入口：宠物工作区所有者现在可从首屏“准备就诊”直接打开已选中的健康摘要表单，编辑者/只查看成员不会看到无权限的入口；摘要主流程回归改为覆盖该入口，桌面/移动 2/2 通过。
- 2026-09-15 摘要临床信息层级补强：Vet-ready PDF 和公开摘要把过敏置于顶部醒目标记，并从同一时间线事实提取体重趋势、疫苗与就诊记录；无记录时明确显示未记录，Landing 构建与 7 项渲染测试通过。
- 2026-09-15 驱虫记录补齐：时间线新增可编辑“驱虫”事件，与疫苗共用名称/备注校验，并纳入 Vet-ready PDF 与公开摘要的疫苗/驱虫区块；后端时间线注册测试、TypeScript、Lint、契约检查和 Landing 7/7 通过。
- 2026-09-15 结构化导出修复：宠物“导出数据”在 Web 直接下载 JSON，原生先写入缓存文件再走系统分享，分享不可用时才复制文本；不再把结构化导出伪装成普通消息分享。桌面/移动下载回归 2/2、TypeScript、Lint、Web export 与契约检查通过。
- 2026-09-15 摘要内容选择补齐：健康摘要创建表单现在逐项控制宠物档案、当前用药和近期记录，并把真实 `sections` 写入分享选项；取消全部内容会被拦截，避免生成没有可读内容的摘要。创建前新增摘要预览，可返回修改主诉、范围和隐私选项；PDF 也按快照中的 section 存在性省略未选择区块，不再渲染伪空占位。桌面/移动摘要分享回归 2/2、TypeScript 与 Lint 通过。
...
- 2026-09-15 保存后刷新错误可见性补强：家庭编辑、管理员转让、照护计划和用药表单不再把异步刷新回调当成同步调用；写入成功后刷新失败会被捕获并以中文 Toast 暴露，用户仍可通过页面重试，而不会出现未处理 Promise 或静默误报。TypeScript、Lint、前端产品契约检查与 PlanetBuildCheck Simulator 启动复核通过。
- 2026-09-15 写入后回读状态补强：家庭成员/宠物关系、成员角色、宠物归档恢复、负责人排序、通知偏好、账户资料和会话管理在服务端写入后都检查 React Query 回读结果；回读失败时明确提示“已保存但页面刷新失败”，不再把旧缓存当成新状态显示。TypeScript、Lint 与前端产品契约检查通过。
- 2026-09-15 回读失败覆盖扩展：宠物家庭关联、家庭转移列表、已删除宠物/家庭恢复、成员治理、负责人排序和通知偏好等写入口均补上回读错误分支；写入成功但缓存刷新失败时保留成功事实并提示刷新失败，页面继续提供 QueryErrorState 重试路径。API walkthrough 51/51、TypeScript、Lint 与前端产品契约检查通过。
- 2026-09-15 通知动作失败可见性补强：原生通知上的接受/拒绝在网络失败且本地队列也不可写时，SessionProvider 记录具体请求/批次失败并在详情与列表显示可读告警；用户可关闭告警并使用页面内真实操作重试，切换账户或退出登录时自动清理。TypeScript、Lint 与前端产品契约检查通过。
- 2026-09-15 通知失败契约守门：前端产品契约脚本固定检查通知动作失败状态同时接入单项请求与批量安排页面，防止后续 UI 改动重新引入静默失败。
