...
import { humanDisplayName } from '../src/core/presentation/labels';
...
export default function AccountRoute() {
  ...
  useEffect(() => { if (!nameDirty) setName(humanDisplayName(me.data?.user) ?? ''); }, [me.data?.user, nameDirty]);
  const update = useMutation({ mutationFn: () => planetApi.me.update(name.trim()), onSuccess: () => { setNameDirty(false); setMessage('Profile saved.'); invalidate.me(); }, onError: (err) => setMessage(err instanceof ApiError ? err.message : 'Unable to save profile.') });
