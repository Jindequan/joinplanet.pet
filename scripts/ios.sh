...
if [ -n "${UDID:-}" ]; then
  ...
  # Xcode-beta 27 moved the Simulator window into DeviceHub.app. Prefer it
  # when present, then retain compatibility with stable Xcode installations.
  ...
  if [ -d "$DEVICE_HUB_APP" ]; then
    open -a "$DEVICE_HUB_APP" >/dev/null 2>&1 || true
  elif [ -d "$SIM_APP" ]; then
...
if [ "$METRO_ALREADY_UP" = 0 ]; then
  ...
  if [ -f "$ROOT_DIR/.dev/frontend.log" ]; then
    mv -f "$ROOT_DIR/.dev/frontend.log" "$ROOT_DIR/.dev/frontend.log.previous" 2>/dev/null || true
  fi
  : > "$ROOT_DIR/.dev/frontend.log"
