...
import { AppText, Button, Card, LoadingState, QueryErrorState, Screen, ViewFilterBar, type ViewFilter } from '../../src/ui/components';
...
export default function TodayRoute() {
  ...
  return <Screen scroll contentContainerStyle={styles.content}>
    ...
    <View style={styles.filterRow}><View style={styles.filterGrow}><ViewFilterBar value={effectiveFilter} families={families} pets={accessiblePets.pets} onChange={setFilter} /></View><Pressable accessibilityRole="button" accessibilityLabel={accessiblePets.pets.length === 1 ? 'Add care' : 'Choose a Pet for care'} onPress={openCareSetup} style={({ pressed }) => [styles.addButton, { backgroundColor: theme.colors.brandStrong }, pressed && { opacity: theme.motion.pressOpacity }]}><PlusIcon size={17} color={theme.colors.onBrand} weight="bold" /><AppText variant="label" style={{ color: theme.colors.onBrand }}>Add</AppText></Pressable></View>
...
const styles = StyleSheet.create({
  ...
  progressRing: { width: 86, height: 86, borderRadius: 43, borderWidth: 9, alignItems: 'center', justifyContent: 'center' },
