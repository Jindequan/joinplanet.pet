...
- `GET /me` → `{user, circles:[{id,name,timezone,role,pet:{id,name,species,breed,birthday,avatar_key}}], entitlements:["*"|"..."]}`

## F2 圈子

- `POST /circles` `{pet_name, species?, breed?, timezone?}` → 建 circle+pet+owner 一体 `{circle:{id,name,timezone,invite_code}, pet:{...}}`
- `POST /circles/join` `{invite_code}` → 以 caregiver 加入 `{circle, pet}`
...
- `POST /pets/{petID}/attachments` multipart `file` + `event_id?` → 存本地 `./uploads/{random32hex}.{ext}` → `{attachment:{id,kind,url}}`
