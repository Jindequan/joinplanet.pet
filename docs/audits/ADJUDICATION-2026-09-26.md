...
**裁决：✅ 原则性裁决（2026-09-26）**：DB 永远 UTC，时区只在用户层与服务层——已立法 PRODUCT.md/ARCHITECTURE.md；现状实现（规则随迁+物化冻结）符合原则，零代码。
...
**裁决：✅ A（2026-09-26）**：且明确「次日」=用户（家庭）时区次日，非 UTC 次日——实测现状实现已符合（dateAt(date, familyTZ)），已立法。
...
**裁决：✅ A（2026-09-26）**：后端收紧，归档宠禁转移——本批落地。
...
**裁决：✅ B（2026-09-26）**：维持无 TTL，手动撤回；出现「已读不回骚扰」场景再议。
...
**裁决：✅ A（2026-09-26）**：全删（删前 grep planet-cli/集成测试排除隐性依赖）——本批落地。
...
**裁决：✅ 升级立项（2026-09-26）**：免费版必须有完整限额体系（宠物数/家庭数/成员数/资源消耗），系统性设计+Codex 核对后报批实施；参考企业坐席额度设计理念。
