...
function PetCard({ pet, intent }: { pet: Pet; intent?: string }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={intent === 'care' ? `Add care for ${pet.name}` : `Open ${pet.name}`} onPress={() => router.push({ pathname: '/(tabs)/pet', params: { petId: pet.id, intent: intent === 'care' ? 'care' : undefined } })} style={({ pressed }) => [styles.petCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }, pressed && { opacity: theme.motion.pressOpacity }]}>
