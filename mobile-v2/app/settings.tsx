...
import { useFamilies, useInvalidateApi, useMe, useNotificationPrefs } from '../src/core/query/hooks';
...
export default function SettingsRoute() {
  ...
  const me = useMe();
  const [activeFamilyId, setActiveFamilyId] = React.useState<string>();
  ...
  const familyId = families.data?.families.find((family) => family.id === activeFamilyId)?.id ?? families.data?.families.find((family) => family.id === preferredFamilyId)?.id ?? families.data?.families[0]?.id;
  const prefs = useNotificationPrefs(familyId);
  const invalidate = useInvalidateApi();
  React.useEffect(() => { if (prefs.data?.prefs) setDraft(prefs.data.prefs); }, [prefs.data?.prefs]);
  const updatePrefs = useMutation({ mutationFn: (body: { reminders?: boolean; digest?: boolean; alerts?: boolean }) => planetApi.families.updateNotificationPrefs(familyId!, body), onSuccess: () => { if (familyId) invalidate.notificationPrefs(familyId); setSaveMessage('Saved'); }, onError: () => setSaveMessage('We could not save that preference. Try again.') });
  if (families.isLoading || (familyId && prefs.isLoading)) return <Screen><LoadingState label="Loading your settings" /></Screen>;
  if (families.isError || prefs.isError) return <Screen contentContainerStyle={styles.center}><AppText variant="title">Settings are unavailable</AppText><AppText muted>We could not load your notification preferences.</AppText><Button label="Try again" onPress={() => { void families.refetch(); void prefs.refetch(); }} /></Screen>;
  if (!familyId) return <Screen scroll contentContainerStyle={styles.content}><PageHeader eyebrow="YOU / NOTIFICATIONS" title="Notifications" /><Card style={styles.emptyCard}><View style={[styles.heroIcon, { backgroundColor: theme.colors.accentSurface }]}><ShieldCheckIcon size={24} color={theme.colors.brandStrong} weight="duotone" /></View><AppText variant="heading">Create or join a Family first</AppText><AppText muted>Notification preferences belong to a Family. Once you have a shared care space, you can choose which care updates reach you.</AppText><Button label="Open Family" onPress={() => router.push('/(tabs)/family')} /></Card><Button label="Sign out" variant="ghost" icon={<SignOutIcon size={18} color={theme.colors.danger} weight="regular" />} onPress={() => void signOut()} /></Screen>;
